...
class SwaggerConfig {
    ...
    OpenAPI openApi(SwaggerProperties properties) {
        ...
        Info info = new Info()
                ...
                .contact(contact)
                .license(license);
        Server server = new Server().url("http://localhost:8081").description("ProductService Server URL");

        List<Tag> tags = List.of(new Tag().name("Products API").description("API endpoints to manage products"));
        return new OpenAPI()
                .info(info)
